...
scalar_to_color = [
    "y"
]  # Scalar field to color the isosurface, if we want solid color put ''
...
solid_color = [[68 / 255.0, 107 / 255.0, 242 / 255.0], [0, 0, 0]]  # Solid color in RGB
...
if __name__ == "__main__":
    ...
    for it in range(nt):
        ...
        startx, starty, startz, midx, midy, midz, endx, endy, endz = compute_bounds(
            x, y, z
        )

        renderView1 = setup_render_view(
            startx,
            midx,
            midy,
            endy,
        ...
        netcdfSource1 = netcdfSource(
            DataFileName=f"{data_dir}/{data_fname}",
        ...
        for iS in range(len(isosurface_fields)):
            ...
            contourDisplay.append(
                Show(contour[iS], renderView1, "GeometryRepresentation")
            )
        ...
        # Save Screenshot
        SaveScreenshot(
            f"{imag_dir}/{imag_name}",
            ImageResolution=(resolution[0], resolution[1]),
